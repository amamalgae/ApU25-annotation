...
§4–§6 では集計を 2 通り並記しているが、**どちらも上記 3 つの交絡を
除去していない**。とくに 1（Pfam リリース差）については、eggNOG 側の
Pfam 版が特定不能である以上、**両側の共通集合を定義すること自体ができない**。
2 通りの違いは、eggNOG 側の呼び出しを一部除外するかどうかだけである（§4）。
...
ただし一致するのは InterPro 側と対応表の間だけである。eggNOG 側の版は不明のままなので、**「両側に共通する Pfam 版」は定義できていない**。§4 以降で行っているのは、Pfam 38.2 の名前表で解決できない eggNOG 側の呼び出しを除外することだけで、バージョン差の除去ではない。
...
集計は 2 通り。**どちらもバージョン差を除去していない**（§0、§2.2）。

- **全呼び出し**: キーは Pfam 名。eggNOG の呼び出しを 1 つも落とさない。
- **対応表で解決できた呼び出しのみ**: キーは PF アクセッション。Pfam 38.2 の名前表で解決できない eggNOG 側の呼び出しを除外する。**除外されるのは eggNOG 側だけ**である。

| 区分 | 全呼び出し | 対応表で解決できた呼び出しのみ |
|---|---|---|
| 両方にある | 3544 | 3544 |
...
**この操作は「両側を同じ Pfam 版に揃える」ものではない。**
片側（eggNOG）の分母を減らしているだけであり、InterPro 側は 1 呼び出しも減っていない。

## 5. InterProScan 6 を正解としたときの eggNOG の性能

| 指標 | 全呼び出し | 対応表で解決できた呼び出しのみ |
|---|---|---|
...
「両方にある」と「InterPro のみ」は 2 通りで**同数**である。
すなわち §1 で変換できなかった eggNOG 名は 1 つも InterPro 側と一致していない。
よって recall は 83.51 % のまま動かず、
**F1 が 82.79 % → 83.87 % に上がったのは、
precision の分母（eggNOG 側の呼び出し総数）が 4317 → 4207 に減ったためである**。
バージョン差が除かれた結果ではない。
...
## 7. QC フラグ別の層別集計（対応表で解決できた呼び出しのみ）
...
本データでの実測は、対応表で解決できた呼び出しのみで F1 = **83.87 %**（precision 84.24 % / recall 83.51 %、対象 3580 タンパク質）、全呼び出しで F1 = **82.79 %** である。
§4 のとおりこの 2 値の差はバージョン差の除去によるものではないので、**83.87 % も一致率の下限**であり、以下の理由により文献値との直接比較はできない。
