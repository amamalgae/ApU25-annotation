...
本書の計時（§6）では `scripts/09_sample_unmatched.py --mask-internal-stop X` で
`*` を `X` に置換した。残基数が変わらないので外挿の前提を壊さない。
**これは計測のための処理であり、本番でどう扱うか（マスクする / 分割する / 除外する）は
未決である。** 本書は選択肢を評価していない。
